...
type ShippingInfo = {
    ...
    discountedPrice?: {               // present only when discount applied
...
const subtotalCentAmount = lineItems.reduce((sum, item) => {
    const priceToUse =
        item.price.discounted?.value.centAmount  // use discounted if exists
        ?? item.price.value.centAmount;           // fallback to original
